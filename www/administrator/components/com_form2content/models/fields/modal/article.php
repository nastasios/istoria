	...
	protected function getInput()
	{
		...
		if ((int) $this->value > 0)
		{
			...
			$query = $db->getQuery(true)
				...
				->from('#__content c')
				->join('LEFT', '#__f2c_form f ON c.id = f.reference_id')
				->select('f.id AS f2cId')
				->where($db->quoteName('c.id') . ' = ' . (int) $this->value);
			$db->setQuery($query);
...
//				$title = $db->loadResult();
		...
		if ($allowEdit)
		{
			if($f2cId)
			{
				...
				$html[] = '<a class="btn hasTooltip' . ($f2cId ? '' : ' hidden') . '" href="index.php?option=com_form2content&layout=modal&tmpl=component&task=form.edit&id=' . $f2cId . '" target="_blank" title="' . JHtml::tooltipText('COM_CONTENT_EDIT_ARTICLE') . '" ><span class="icon-edit"></span> ' . JText::_('JACTION_EDIT') . '</a>';
			}
			else
			{
				// Joomla article, Edit article button
