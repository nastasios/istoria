...
{	
	...
	public function render($translatedFields, $contentTypeSettings, $parms = array(), $form, $formId)
	{
		...
		if($displayValue)
		{
			$date 		= new JDate($displayValue);
			$dateFormat = str_replace('%', '', $this->f2cConfig->get('date_format'));
			$displayValue 		= $date->format($dateFormat);			
		}
		
		$displayData['attributes']		= $this->settings->get('dat_attributes') ? $this->settings->get('dat_attributes') : 'class="inputbox"';
