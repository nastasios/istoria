...
class Form2ContentViewForm extends JViewLegacy
{
	...
	function display($tpl = null)
	{
		...
		$this->assoc = JLanguageAssociations::isEnabled() && $this->contentTypeSettings->get('associations_front_end', 0);
	...
	protected function renderFormTemplate()
	{
		...
		$formVars = array('F2C_ID' => 'F2C_ID', 'F2C_TITLE' => 'F2C_TITLE', 'F2C_TITLE_ALIAS' => 'F2C_TITLE_ALIAS',
						  ...
						  'F2C_ASSOCIATIONS' => 'F2C_ASSOCIATIONS');
		...
		// Add asociations
		$title = 'F2C_ASSOCIATIONS';
				
		if($this->contentTypeSettings->get('associations_front_end', 0))
		{
			if(array_key_exists($title, $varsInTemplate))
			{
				$parser->addVar($title.'_CAPTION', $this->contentTypeSettings->get('associations_caption', JText::_('JGLOBAL_FIELDSET_ASSOCIATIONS')));
				$parser->addVar($title, $this->loadTemplate('associations'));
			}
			else 
			{
