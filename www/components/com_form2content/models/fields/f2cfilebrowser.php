 ...
 * @since       1.6
 */
class JFormFieldF2cFileBrowser extends JFormField
{
	/**
	 * The form field type.
	 *
	...
	{
		$selectionrequired = (string)$this->element['selectionrequired'];
		$this->selectionrequired = ($selectionrequired == 'true' || $selectionrequired == 'required' || $selectionrequired == '1');
		
		$arrElmId 		= explode('_', $this->id);
		$fieldId 		= substr($arrElmId[1], 1);
		$contentType	= F2cFactory::getContentType($this->form->getValue('projectid'));
		...
		if ($this->element['disabled'] != true)
		{
			...
			{				
				$html[] = '<script type="text/javascript">var f2cfield'.$fieldName.'={id:'.$field->id.', fieldtypeid:'.$field->fieldtypeid.', contenttypeid:'.$field->projectid.'};</script>';
				$html[] = '<a class="btn hasTooltip" title="' . JText::_('COM_FORM2CONTENT_DELETE_IMAGE') . '"' . ' href="#" onclick="';
				$html[] = 'deleteUploadedFile(f2cfield'.$fieldName.');';
				$html[] = 'return false;';
				$html[] = '">';
